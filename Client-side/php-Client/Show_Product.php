...
<head>
    ...
    <script>
        async function updateQuantity(rowId, increment) {
                try {
                    const response = await fetch('update_quantity.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ id_product: rowId, increment: increment })
                    });
                    const result = await response.json();
                    if (result.status === 'success') {
                        window.location.reload(); // Reload the page without showing an alert
                    }
                } catch (error) {
        ...
        function editRow(rowId) {
            ...
            editButton.onclick = () => saveRow(rowId);
        }
        ...
        async function saveRow(rowId) {
            ...
            const updatedData = {
                ...
                unit_price: inputs[1].value,
                date_exp: inputs[2].value,
        ...
        if (confirm('Are you sure you want to delete this product?')) {
            try {
                const response = await fetch('delete_product.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id_product: rowId })
                });
                ...
                if (!response.ok) {
                    ...
                    alert('Error deleting product, HTTP status: ' + response.status);
                    return;
                }
                ...
                if (result.status === 'success') {
                    alert(result.message); // Alert only for delete action
...
<nav id="sidebar">
    ...
    <div class="container">
        ...
        <!-- Filter Form -->
        <form method="GET">
            <input type="text" name="product_name" placeholder="Product Name" value="<?= htmlspecialchars($_GET['product_name'] ?? '') ?>" />
            ...
            <select name="supplier_id">
                ...
                <?php
                $suppliers = $conn->query("SELECT id_supplier, CONCAT(first_name, ' ', last_name) AS supplier_name FROM suppliers");
                while ($supplier = $suppliers->fetch_assoc()) {
                    $selected = ($_GET['supplier_id'] ?? '') == $supplier['id_supplier'] ? 'selected' : '';
        ...
        <table>
            ...
            if (!empty($_GET['supplier_id'])) {
                ...
                $where .= " AND p.supplier_id = $supplierId";
            }

            // Query with filters
            $sql = "SELECT p.id_product, p.product_name, p.unit_price, p.date_exp, CONCAT(s.first_name, ' ', s.last_name) AS supplier_name, c.name AS category_name, p.quantity
                    FROM products p
                    ...
                    echo "<tr id='row-{$row['id_product']}'>
                        ...
                        <td>
                            <div class='quantity-actions'>
                                <button onclick='updateQuantity({$row['id_product']}, -1)'>-</button>
                                <span>{$row['quantity']}</span>
                                <button onclick='updateQuantity({$row['id_product']}, 1)'>+</button>
                            </div>
                        </td>
                        <td class='actions'>
                            <button class='edit' onclick='editRow({$row['id_product']})'>Edit</button>
                            <button class='delete' onclick='deleteRow({$row['id_product']})'>Delete</button>
                        </td>
                    </tr>";
                }
            } else {
