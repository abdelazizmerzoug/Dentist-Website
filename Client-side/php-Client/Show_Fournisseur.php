...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit'])) {
    ...
    $stmt = $conn->prepare("UPDATE suppliers SET first_name = ?, last_name = ?, phone = ?, email = ? WHERE id_supplier = ?");
    $stmt->bind_param("ssssi", $first_name, $last_name, $phone, $email, $edit_id);
    $stmt->execute();
    $stmt->close();
    header("Location: " . $_SERVER['PHP_SELF']);
    exit();
...
if (!empty($filter_name)) {
    $where .= " AND first_name LIKE '%" . $conn->real_escape_string($filter_name) . "%'";
}
if (!empty($filter_last_name)) {
    $where .= " AND last_name LIKE '%" . $conn->real_escape_string($filter_last_name) . "%'";
}
if (!empty($filter_email)) {
    $where .= " AND email LIKE '%" . $conn->real_escape_string($filter_email) . "%'";
}
if (!empty($filter_phone)) {
    $where .= " AND phone LIKE '%" . $conn->real_escape_string($filter_phone) . "%'";
}

// Fetch suppliers
$sql = "SELECT * FROM suppliers WHERE $where";
...
<nav id="sidebar">
    ...
    <div class="container">
        ...
        <form method="GET" class="filter-form">
            <input type="text" name="filter_name" placeholder="First Name" value="<?= htmlspecialchars($_GET['filter_name'] ?? '') ?>">
            <input type="text" name="filter_last_name" placeholder="Last Name" value="<?= htmlspecialchars($_GET['filter_last_name'] ?? '') ?>">
            <input type="text" name="filter_email" placeholder="Email" value="<?= htmlspecialchars($_GET['filter_email'] ?? '') ?>">
            <input type="text" name="filter_phone" placeholder="Phone Number" value="<?= htmlspecialchars($_GET['filter_phone'] ?? '') ?>">
            ...
            <?php if ($result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td class="actions">
                            <button class="edit" onclick="toggleEditForm(<?= $row['id_supplier'] ?>)">Edit</button>
                            <form method="POST" id="delete-form-<?= $row['id_supplier'] ?>" style="display:inline;">
                                <input type="hidden" name="id" value="<?= $row['id_supplier'] ?>">
                                <button type="button" class="delete" onclick="confirmDelete(<?= $row['id_supplier'] ?>)">Delete</button>
                            </form>
                        </td>
                    </tr>
                    <tr id="edit-form-<?= $row['id_supplier'] ?>" style="display:none;">
                        <td colspan="6">
